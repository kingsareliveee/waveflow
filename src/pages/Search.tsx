...
import { Search as SearchIcon, X, Clock, TrendingUp, Trash2, ChevronRight, Music, Mic2, ListMusic } from "lucide-react";
...
const GENRES = [
  // Indian
  "Punjabi Pop", "Bollywood Hits", "Desi Hip-Hop", "Sufi Music",
  "Urdu Rap", "Indie Hindi", "Tamil Beats", "Bhangra",
  // International
  "Lofi Hip Hop", "Synthwave", "Ambient", "Jazz Fusion",
  "Post-Rock", "Indie Folk", "Electronic", "R&B",
];

const TRENDING_SEARCHES = ["Karan Aujla", "Sidhu Moosewala", "Diljit Dosanjh", "Shubh", "Talha Anjum"];

const TRENDING_ARTISTS = [
  { name: "Arijit Singh", thumbnail: "https://yt3.googleusercontent.com/DcEzZrPCQRSSs47rMbdJ3UJkQUCN3X8SKf8aCnvOgd2BmPihAz-0jBGJgEVh9_P8EiSBVNyixDs=s88-c-k-c0x00ffffff-no-rj-mo" },
  { name: "Karan Aujla", thumbnail: "https://yt3.googleusercontent.com/Da4zbrS4XLxzb3xVNT14aKr22aBg1blJCuCBppbYglO_uDmElYopgoDk7XV6UWNxthI96XOYrw=s88-c-k-c0x00ffffff-no-rj-mo" },
  { name: "Sidhu Moosewala", thumbnail: "https://yt3.ggpht.com/ytc/AIdro_kiQJ0Hhp0O-tdaY1dy81-gSNujjccUlWstnpFr686ZlMk=s88-c-k-c0x00ffffff-no-rj-mo" }
];

const TRENDING_PLAYLISTS = [
  { id: "PLRNsqJKPXrnmruVt4-BN4rTYiUmz2jeCF", title: "Moosetape", author: "Sidhu Moose Wala", thumbnail: "https://i.ytimg.com/vi/y-abKv4oqjk/hq720.jpg?sqp=-oaymwEdCNAFEJQDSFryq4qpAw8IARUAAIhCGAHtAWZmZkI=&rs=AOn4CLAeF7f2BsCkWvsA0iaK4Fmgp4phXw", videoCount: "22 videos" },
  { id: "OLAK5uy_lKFhD4-IEWtRaQ4ZhCCu4-Bo9mTmeDGCc", title: "Moosetape Full Album", author: "Sidhu Moose Wala", thumbnail: "https://i.ytimg.com/vi/y-abKv4oqjk/hq720.jpg?sqp=-oaymwEdCNAFEJQDSFryq4qpAw8IARUAAIhCGAHtAWZmZkI=&rs=AOn4CLAeF7f2BsCkWvsA0iaK4Fmgp4phXw", videoCount: "32 videos" }
];
...
export const Search: React.FC = () => {
  ...
  const [recentItems, setRecentItems] = useState<RecentSearchItem[]>(loadRecentItems);
  ...
  const handlePlaySong = (song: Song, index: number, overrideQueue?: Song[]) => {
    setCurrentSong(song);
  ...
  const handlePlaylistClick = (playlist: any) => {
    ...
    const updated = saveRecentItem({
      ...
      subtitle: playlist.author || "YouTube",
  ...
  const partitioned = React.useMemo(() => {
    ...
    const albums = results.playlists.filter(p =>
  ...
  return (
    <div className="flex flex-col gap-8 pb-12 relative min-h-[70vh]">
                ...
                <div className="flex-1 overflow-y-auto max-h-[280px] pr-1.5 flex flex-col gap-1.5">
                  {recentItems.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center text-white/30 gap-2">
                      ...
                      <span className="text-xs font-medium">No recent searches</span>
                    </div>
                  ) : (
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="flex flex-col min-w-0">
                            ...
                            <span className="text-xs text-white/40 truncate">
                              {item.subtitle || (item.type === "song" ? "Song" : item.type === "artist" ? "Artist" : "Playlist")}
                            </span>
                          </div>
                        </div>
                        <button
      ...
      {/* ── SEARCH RESULTS ───────────────────────────────── */}
      {hasQuery && !loading && !error && results && (results.songs?.length > 0 || results.playlists?.length > 0 || results.artists?.length > 0) && (
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  ...
                  Songs
                  <span className="text-xs font-normal text-white/30 ml-1">({results.songs.length})</span>
                </h2>
                {!isMobile && results.songs.length > 5 && (
                  ...
                  >
                    ...
                    <ChevronRight className={`w-3.5 h-3.5 transition-transform ${expandedSections["songs"] ? "rotate-90" : ""}`} />
                  </button>
                )}
              </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  ...
                  Artists
                  <span className="text-xs font-normal text-white/30 ml-1">({results.artists.length})</span>
                </h2>
                {!isMobile && results.artists.length > 5 && (
                  ...
                  >
                    ...
                    <ChevronRight className={`w-3.5 h-3.5 transition-transform ${expandedSections["artists"] ? "rotate-90" : ""}`} />
                  </button>
                )}
              </div>
                    ...
                    >
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm font-bold text-white truncate flex items-center gap-1">
                          {artist.name}
                          {artist.verified && <span className="text-[9px] bg-blue-500 rounded-full px-1 py-0.5 text-white">✓</span>}
                        </span>
                        <span className="text-xs text-white/40 truncate">Artist</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  ...
                  Albums & EPs
                  <span className="text-xs font-normal text-white/30 ml-1">({partitioned.albums.length})</span>
                </h2>
                {!isMobile && partitioned.albums.length > 5 && (
                  ...
                  >
                    ...
                    <ChevronRight className={`w-3.5 h-3.5 transition-transform ${expandedSections["albums"] ? "rotate-90" : ""}`} />
                  </button>
                )}
              </div>
                  ...
                  {partitioned.albums.map((album) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-xs text-white/40 truncate">{album.author} • Album</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <ListMusic className="w-4.5 h-4.5 text-accent" />
                  Playlists
                  <span className="text-xs font-normal text-white/30 ml-1">({partitioned.playlists.length})</span>
                </h2>
                {!isMobile && partitioned.playlists.length > 5 && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-bold text-accent hover:underline"
                  >
                    {expandedSections["playlists"] ? "Show Less" : "Show More"}
                    <ChevronRight className={`w-3.5 h-3.5 transition-transform ${expandedSections["playlists"] ? "rotate-90" : ""}`} />
                  </button>
                )}
              </div>
                  ...
                  {partitioned.playlists.map((playlist) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-xs text-white/40 truncate">By {playlist.author} • Playlist</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {/* ── NO RESULTS STATE ─────────────────────────────── */}
      {hasQuery && !loading && !error && results && (!results.songs?.length && !results.playlists?.length && !results.artists?.length) && (
        <motion.div
          key="no-results"
        ...
        >
          <div
            className="w-16 h-16 rounded-2xl flex items-center justify-center bg-white/4"
          >
            <SearchIcon className="w-6 h-6 text-white/20" />
          </div>
          <div>
            <p className="font-semibold text-white/50">No results for "{query}"</p>
            <p className="text-sm text-white/25 mt-1">Try a different search query</p>
          </div>
        </motion.div>
      )}

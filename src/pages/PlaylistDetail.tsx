...
import {
  Play, Heart, Shuffle, ArrowLeft, Music, Trash2, ListMusic, Download, Loader2
} from "lucide-react";
...
export const PlaylistDetail: React.FC = () => {
  ...
  const isYoutubePlaylist = id ? !isUuid(id) : false;
  ...
  // Fetch YouTube playlist if applicable
  useEffect(() => {
    if (!id || !isYoutubePlaylist) return;
    ...
    fetchYtPlaylist();
  }, [id, isYoutubePlaylist]);

  // Check if YouTube playlist is liked in local storage
  useEffect(() => {
    if (!id || !isYoutubePlaylist) return;
  ...
  const songList = isYoutubePlaylist ? (ytPlaylist?.songs || []) : (dbPlaylist?.songs || []);
  const count = songList.length;
  const name = isYoutubePlaylist ? (ytPlaylist?.title || "YouTube Playlist") : (dbPlaylist?.name || "");
  const author = isYoutubePlaylist ? (ytPlaylist?.author || "YouTube") : "You";
  const thumbnail = isYoutubePlaylist ? (ytPlaylist?.thumbnail || "") : (songList[0]?.thumbnail || "");
  ...
  const importPlaylist = async () => {
    ...
    try {
      ...
      const songsToInsert = ytPlaylist.songs.map((song: Song, index: number) => ({
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center">
        <ListMusic className="w-12 h-12" style={{ color: "rgba(255,255,255,0.15)" }} />
        <p className="text-sm text-white/30">Sign in to see your playlists</p>
      </div>
    );
  }

  if (isYoutubePlaylist && loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center">
        <Loader2 className="w-8 h-8 animate-spin text-accent" />
        <p className="text-sm text-white/40">Loading YouTube Playlist...</p>
      </div>
    );
  }

  if (isYoutubePlaylist && error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center">
        <p className="text-sm text-red-400">{error}</p>
        <button onClick={() => navigate(-1)} className="text-sm text-accent underline">
          Go Back
        </button>
      </div>
    );
  ...
  if (!isYoutubePlaylist && !dbPlaylist) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center">
        <p className="text-sm text-white/30">Playlist not found</p>
        <button onClick={() => navigate("/library")} className="text-sm text-accent underline">
          Go back to Library
        </button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-8 pb-8">
      ...
      <motion.div
        ...
        style={{
          background: "linear-gradient(135deg, rgba(95,168,255,0.15) 0%, rgba(139,207,198,0.10) 50%, rgba(255,255,255,0.02) 100%)",
      ...
      >
        ...
        <div
          ...
          style={{
            background: "radial-gradient(circle, rgba(95,168,255,0.2) 0%, transparent 70%)",
            filter: "blur(50px)",
          }}
        />
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end gap-6">
          ...
          <div
            ...
            style={{
              ...
              boxShadow: "0 16px 48px rgba(95,168,255,0.25), 0 8px 24px rgba(0,0,0,0.4)",
            }}
          >
            {thumbnail ? (
          ...
          <div className="flex flex-col gap-2 text-left">
            <span className="text-xs font-semibold uppercase tracking-[0.15em] text-white/30">
              {isYoutubePlaylist ? "YouTube Playlist" : "Playlist"}
            </span>
            <h1
          ...
          {isYoutubePlaylist && (
            <>
              ...
              <motion.button
                ...
                style={{
                  background: isLiked ? "rgba(var(--accent-rgb),0.10)" : "rgba(255,255,255,0.05)",
                  borderColor: isLiked ? "var(--accent)" : "rgba(255,255,255,0.1)",
      ...
      {count === 0 ? (
        ...
        >
          ...
          <div>
            <p className="font-semibold text-white/40 mb-1">Playlist is empty</p>
            <p className="text-sm text-white/25">
              {isYoutubePlaylist ? "No tracks found in this YouTube playlist." : "Search for tracks and add them using the context menu"}
            </p>
          </div>
        </motion.div>
      ) : (
